...
export function buildStructuredReplyInstructions(policy: ChannelPolicy): string {
  ...
  return [
    "输出协议：",
    "1. 你必须只输出一个 JSON 对象，禁止输出 Markdown 代码块、解释、前后缀文本。",
    "2. JSON 结构固定为 {\"content\":\"...\",\"intent\":null|{...}}。",
    "3. content 必须是准备发回频道的中文正文，不要包含 @mention 协议标记。",
    "4. intent 只允许包含 task_type、priority、requires_approval、deadline、tags；只要 intent 非空，就必须包含 task_type。",
    "5. 如果 intent 非空，必须同时输出 task_type、priority、requires_approval、deadline、tags 这 5 个键；没有值的键必须显式填 null。",
    "",
    "当前频道策略：",
    ...
    "intent 判定规则：",
    intentRule,
    "- 明确向对方索取信息、判断、确认、结论时，用 task_type=\"question\"。",
    "- 明确要求对方执行动作、调查、实现、验证、跟进时，用 task_type=\"task_assignment\"。",
    "- 明确要求审查代码、Patch、实现方案时，用 task_type=\"code_review\"。",
    "- 明确请求批准、授权、放行时，用 task_type=\"approval_request\"；只有确实在请求审批时才设置 requires_approval=true。",
    "- 明确输出方案选择、拍板、裁定、最终结论时，用 task_type=\"decision\"。",
    "- 明确同步日志发现、调查结果、事实结论、上下文信息时，用 task_type=\"info_share\"。",
    "- 明确报告问题、异常、故障、回归时，用 task_type=\"bug_report\"。",
    "- 明确提出新增能力或变更需求时，用 task_type=\"feature_request\"。",
...
/**
 * 校验并清理模型生成的 intent。
 * @param rawIntent 模型返回的原始 intent。
 * @param policy 当前频道策略。
 * @returns 归一化后的 intent。
 */
function normalizeStructuredIntent(rawIntent: unknown, policy: ChannelPolicy): MessageIntent | null {
  ...
  if (rawIntent.priority !== undefined) {
    if (rawIntent.priority === null) {
      // Structured Outputs 要求对象分支把可选字段补齐为 null，这里将其视为“未提供”。
    } else
  ...
  if (rawIntent.requires_approval !== undefined) {
    if (rawIntent.requires_approval === null) {
      // Structured Outputs 要求对象分支把可选字段补齐为 null，这里将其视为“未提供”。
    } else
