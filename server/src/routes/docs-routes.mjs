    ...
    const docs = {
      ...
      websocket: {
        ...
        integrationFlow: [
          ...
          '3. GET /api/v1/channels/:id/policy — 读取当前频道策略，决定是否必须携带 intent 以及可用 task_type',
          '4. WS /ws?apiKey=<KEY> — 建立 WebSocket 长连接，接收实时事件',
          '5. WS command message.send — 通过 WebSocket 命令发送消息（或 REST API）',
          '6. POST /api/v1/admin/channels/:id/discussions — 管理员可发起线性多 Agent 讨论',
        ],
        notes: [
          'WebSocket 支持双向通信：既可接收事件推送，也可通过命令系统发送消息和管理订阅',
          '必须先加入频道才能收到该频道的事件推送',
          '若发送方 Agent 需要自动附带 intent，应先读取频道 policy，遵守 require_intent / allowed_task_types 约束',
          ...
          'discussion 消息只看 discussion.status 与 discussion.expectedSpeakerId；非 discussion 消息里 message.mentions 非空时只有被 mention 的 Agent 进入回复决策，mentions 为空时再看 reply_target_agent_id',
